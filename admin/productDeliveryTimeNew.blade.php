    ...
    <table id="products-table" class="tablesorter table table-hover table-bordered" style="vertical-align: middle;">
        ...
        @foreach($productList as $product)
            ...
            <tr style="@if($product->activation_status_id == 1)
                ...
                {!! Form::model($product, ['action' => 'AdminPanelController@updateProductDeliveryTime', 'files'=>true, 'id' => $product->id ]) !!}
                    ...
                    <td>
                        {!! Form::hidden('city_id', $product->city_id, ['class' => 'form-control']) !!}
                ...
                <tr style="@if($product->activation_status_id == 1)
                    ...
                    <td data-order="{{$product->activation_status_id}}">
                        <div class="form-group">
                            {!! Html::link('/admin/product-delivery-time/' . $product->id  . '/' . $product->city_id  , 'Değiştir', ['class' => 'btn btn-primary form-control']) !!}
                        </div>
                    </td>
                </tr>
            @endif
