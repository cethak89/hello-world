...
@section('content')
    ...
    <tr>
        <td>
            <h1>Bloom & Fresh Şirket Teslimat Listesi</h1>
        </td>
    </tr>
    </table>
    @if( $errors->any() )
       ...
       <div class="modal-dialog" role="document">
           <div class="modal-content">
               <div class="modal-header">
                ...
                {!! Form::model(null, ['url' => '/admin/companyDeliveryComplete', 'files'=>true, 'accept' => 'image/jpeg,image/gif,image/png,application/pdf,image/x-eps'  , 'method' => 'post']) !!}
   ...
   <button id="test1" type="button" class="btn btn-primary hidden" data-toggle="modal" data-target="#exampleModal" data-whatever="@getbootstrap">Open modal for</button>
    <table id="products-table" class="table table-hover table-bordered" style="vertical-align: middle;">
        ...
        @foreach($deliveryList as $delivery)
            <tr>
                <input id="receiver" type="text" class="form-control hidden" value="{{$delivery->receiver}}">
                <td style="width: 20%;">{{$delivery->id}}</td>
                <td style="width: 20%;">{{$delivery->delivery_location}}</td>
                <td style="width: 20%;">{{$delivery->receiver}}</td>
                <td style="width: 20%;">{{$delivery->product_name}}</td>
                <td style="width: 20%;">
