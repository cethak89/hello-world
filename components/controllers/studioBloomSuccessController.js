...
var studioBloomSuccessModule = angular.module("studioBloomSuccess",
    [
        'menuModule',
        'footerModule',
...
studioBloomSuccessModule.controller("studioBloomSuccessController", function ($timeout,$scope, $http,$sce, $cookies, $stateParams, $state,momentHelper, facebookhelper, userAccount, adwordsHelper, flowerFactory, analyticsHelper, PageTagsFactory) {
    $scope.canEditBilling = true;

    if ($stateParams.orderId === undefined) {
        $state.go('landing');
    } else {
        $http.get(webServer + '/studio-bloom-success-data/' + $stateParams.orderId)
            .success(function (response) {
                var momentObjStart = momentHelper.getTime('YYYY-MM-DD HH:mm:ss',moment(response.data.wanted_date));
                $scope.receiveDate = momentHelper.getTime('DD MMMM YYYY, dddd',momentObjStart);

                var tempPriceSuccess = parseFloat(replaceString(response.data.price, ",", ".")).toFixed(2);
                ...
                $scope.purchaseInfo = {
                    ...
                    flower_desc: response.data.flower_desc
                };
...
studioBloomSuccessModule.controller('StudioPurchaseBillingCtrl', function ($scope, $http, $timeout,$stateParams , $document,errorMessages, userAccount,translateHelper, otherExceptions) {
    $scope.working = false;
    $scope.errorMessage = "";
    $scope.invoice = {};
